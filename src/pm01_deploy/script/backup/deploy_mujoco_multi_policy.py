...
if __name__ == "__main__":
    ...
    config_file="/home/ubuntu/workspace/pm01/deploy/pm01_deploy/src/pm01_deploy/config/pm01_mujoco_multi_policy.yaml"
    ...
    with open(config_file, "r") as f:
        ...
        policy_path_base = config["policy_file_base"]
        policy_pathe_amp = config["policy_file_amp"]
        ...
        base_action_scale = config["base_action_scale"]
        amp_action_scale = config["amp_action_scale"]
        # cmd_scale = np.array(config["cmd_scale"], dtype=np.float32)

        num_actions = 24
        num_obs_base = 425  # config["num_observations"]
        num_obs_amp = 425  # config["num_observations"]
    ...
    obs_base = np.zeros(num_obs_base, dtype=np.float32)
    obs_amp = np.zeros(num_obs_amp, dtype=np.float32)
    ...
    # load policy
    policy_base = torch.jit.load(policy_path_base)        
    policy_amp = torch.jit.load(policy_pathe_amp)        
